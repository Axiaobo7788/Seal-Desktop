...
- [x] Step 6：desktop 侧执行适配（main：YtDlpFetcher + DownloadPlanExecutor + 最小 UI 验证）
  ...
  - [ ] 自定义命令页高级能力（与 Android 对齐）：
    - 模板导入/导出（剪贴板/文件）。
    - 多选与批量删除。
    - 选项快捷键（Shortcut）管理与帮助链接。

  ### 11.2 下载队列（高优先级）
  - [x] 详情弹窗：
    - 展示执行参数、日志尾部、输出路径、文件大小、退出码。
    - 每条任务独立日志/错误输出（非全局日志）。
  - [ ] Resume 支持：
  ...
  - [ ] 下载动作面板能力补齐：
    - [x] “打开所在文件夹”操作与桌面平台适配。
    - [ ] Resume/Cancel 按钮的可用性/提示与运行中状态一致。
  ...
  - [ ] 退出码与错误归因：
    - 将 exit code、stderr 与失败原因持久化到队列项。
...
目标：进入项目完结前的“少量关键差距清零”阶段，不再做大规模架构迁移。

### 13.1 已基本对齐（可视为稳定基线）
- [x] 下载队列主流程（新增任务、状态流转、动作面板、详情弹窗）。
- [x] 桌面端文件动作（打开文件、打开文件位置、复制链接、打开链接、删除队列项）。
- [x] 下载历史页（搜索、筛选、导入导出、行级动作）。
- [x] 自定义格式选择基础能力（拉取格式、分组选择、建议格式、发起下载）。

### 13.2 与 Android 仍有差距（发布前建议补齐）
- [ ] 格式选择高级能力：视频裁剪、按章节拆分、字幕/自动字幕细粒度选择与偏好更新。
- [ ] 下载前状态机深度：Input/Configure/Loading/Error 的失败重试与状态一致性（与 Android 行为对齐）。
- [ ] 历史页批量能力：多选、全选、批量删除（含可选删除本地文件）与计数/体积反馈。
- [ ] 播放列表细分选择：先抓取条目，再按条目范围/勾选生成多任务下载。

### 13.3 明确不做“硬对齐”的部分（平台差异保留）
...
目标：不是只追 Android 对齐，而是形成桌面端独有体验。

### 14.1 高优先级（推荐先做）
- [ ] 拖拽即下载：支持把链接/文本拖入窗口直接创建任务。
- [ ] 快捷键体系：粘贴并新建、聚焦搜索、打开文件、打开文件位置、取消/继续。
- [ ] 系统托盘与后台下载：最小化到托盘、完成通知、失败快速重试。

### 14.2 中优先级
- [ ] 历史页增强：按目录分组、按“文件存在性”快速过滤、批量打开目录。
- [ ] 并发/资源策略：并发下载数、网络限速模板、CPU 占用策略（桌面端更有价值）。
- [ ] 诊断能力：失败任务的一键导出诊断包（参数、stderr 尾部、版本信息）。

### 14.3 低优先级
- [ ] 桌面联动：任务完成后可选执行脚本/命令（用户可配置）。
- [ ] 文件管理集成：常用下载目录收藏、最近目录快速切换。
...
### 15.1 必做（P0）
- [ ] 12.6 截图验收闭环（Android/desktop 关键页对照）。
- [ ] 13.2 中至少完成两项：
  - 格式选择高级能力（字幕/章节）
  - 历史页批量能力
- [ ] 下载动作回归：打开文件/打开位置/删除本地文件在三平台 smoke test 通过。

### 15.2 应做（P1）
- [ ] 14.1 中至少完成一项（建议：拖拽即下载）。
